...
def count_qingti_1l(df: pd.DataFrame) -> int:
    """统计1升装茉莉青提的出品数量"""
    ...
    # 商品名称包含茉莉青提 且 商品规格为1L（排除100ml试饮、听装/瓶装、一打套餐）
    qingti_filter = base_df['商品名称'].astype(str).str.contains('茉莉青提', na=False)
    qingti_filter = qingti_filter & base_df['商品规格'].astype(str).str.contains('1L', na=False)
    qingti_df = base_df[qingti_filter]

    return int(qingti_df['出品数量'].sum())


class QingtiCalculatorApp:
    def __init__(self, root):
        self.root = root
        self.root.title("1升装茉莉青提计算器 - 门店点餐订单报表分析工具")
    ...
    def setup_ui(self):
        ...
        self.result_tree.heading("1升装茉莉青提", text="1升装茉莉青提")
        self.result_tree.column("1升装茉莉青提", width=200, anchor="center")
    ...
    def calculate(self):
        ...
        try:
            ...
            self.status_var.set(f"计算完成，共处理 {len(self.results)} 个门店，1升装茉莉青提合计: {total}")
    ...
    def export_excel(self):
        ...
        filepath = filedialog.asksaveasfilename(
            ...
            initialfile=f"1升装茉莉青提报表_{datetime.now().strftime('%Y%m%d')}.xlsx"
        )
